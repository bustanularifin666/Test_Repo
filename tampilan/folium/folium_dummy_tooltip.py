...
for vehicle_pos in vehicles_positions:
    ...
    tooltip_text = f"Jarak: {distance:.2f} m"
    folium.Marker(vehicle_pos, tooltip= tooltip_text, icon=folium.Icon(color='red')).add_to(map)

# Menyimpan peta ke dalam file HTML
map.save('vehicle_positions_map_tooltip.html')
